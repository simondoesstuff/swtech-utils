<script lang="ts">
  ...
  import {
    evidenceIndexQuery,
    tokenIdFreqMany,
  } from "$lib/could_align/could_align";
  ...
  async function reevaluate() {
    ...
    const qScores = queries.map((q) =>
      evidenceIndexQuery(q, evTokens, tokenSize),
    );

    timing = [t1 - t0, performance.now() - t1];

    annotations = qScores.map((scores) =>
      scores.map((score: number) => {
        let color = score > 0 ? [0, 1, 0.2] : [1, 0, 0.2];
        color = color.map((c) => Math.floor(c * 255));
        return {
          fg: score <= 0.5 ? "white" : "black",
          bg: `rgb(${color.join(",")})`,
          bg_opacity: 0.9 * Math.max(Math.min(score, 1), 0.2),
        };
      }),
    );
  }
  ...
  async function checkSelection(event: MouseEvent) {
    ...
    if (sel) {
      ...
      try {
        floatCancel();
        const promise = new Promise<"up" | "down" | "close">((r, c) => {
          floatResolve = r;
          floatCancel = c;
        });
...
<main class="m-5 lg:m-10">
  <span class="flex flex-col justify-between w-full m-2">
    <div>
      ...
      <span class="font-bold">
        <input class="pl-2 w-10" on:input={parseInput} value={tokenSize} />
      </span>
    </div>
    <div>
  ...
  <div id="queries" class="mb-[7vh]">
    <div class="annotBlock" on:mouseup={checkSelection}>
      {#each queries as q, i}
        {#if q}
          <div>
            <AnnotTextField bind:text={q} bind:annotations={annotations[i]} />
          </div>
        {/if}
      {/each}
    </div>
  ...
  <div id="evidence" class="annotBlock" on:mouseup={checkSelection}>
    {#each evidence as e}
      {#if e}
        <div>
          <AnnotTextField bind:text={e} annotations={[]} />
        </div>
      {/if}
    {/each}
  </div>
